	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			...
			<div class="col-md-10 ">
				...
				<tr>
					<td>${item.id }</td>
					<td>${item.jishuxuqiuname }</td>
					<td>${item.createdate }</td>
					<td>
					<a href="${pageContext.request.contextPath}/user/userZhengjiServlet?method=needshangchuan&id=${item.id }&userid=${user.id}">
					<button type="button" class="btn btn-primary">上传</button></a></td>
					<td>
					<a href="${pageContext.request.contextPath}/user/userZhengjiServlet?method=needdelete&id=${item.id }&userid=${user.id}">
					<button type="button" class="btn btn-primary">删除</button></a></td>
					<td>
					<a href="${pageContext.request.contextPath}/user/userZhengjiServlet?method=needhechadetail&id=${item.id }">
